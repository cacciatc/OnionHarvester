package  {
    ...
    public class Sprintf {
         ...
         */
        public static function format(format:String, values:Array):String{
            ...
            var argsIndex:Number = 0; // argument index
                ...
                while(!isBreak && formatIndex < format.length){
                    ...
                    switch(char = format.charAt(formatIndex)){
                        ...
                        case "d": // signed decimal
                            isBreak = true;
                            result += postFormat(formatDecimal(values[isOrder ? order : argsIndex++], (char == "d")), length, fillSign, adjustment);
                            break;

                        case "f": // float
                            isBreak = true;
                            result += postFormat(formatFloat(values[isOrder ? order : argsIndex++], precision), length, fillSign, adjustment);
                            break;

                        case "s": // string
                            isBreak = true;
                            result += postFormat(formatString(values[isOrder ? order : argsIndex++]), length, FILLSIGN, ADJUSTMENT);
                            break;

                        case "x":
                        case "X":
                            isBreak = true;
                            result += postFormat(formatHex(values[isOrder ? order : argsIndex++], (char == "X")), length, FILLSIGN, ADJUSTMENT);
                            break;

                        case "b":
                            isBreak = true;
                            result += postFormat(formatBin(values[isOrder ? order : argsIndex++]), length, FILLSIGN, ADJUSTMENT);
                            break;

                        case "o":
                            isBreak = true;
                            result += postFormat(formatOct (values[isOrder ? order : argsIndex++]), length, FILLSIGN, ADJUSTMENT);
                            break;
        ...
        public static function formatTime(value:Number):String{
            return Sprintf.format("%02d:%02d" , [value / 60 , value % 60]);
        }
